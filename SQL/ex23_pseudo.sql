...
select 
    ...
    rownum,              -- 의사컬럼 > output > 컬럼의 모습이나 특성과 가장 유사함
    substr(name,2),     -- 함수 > input + output > 객체의 특성에 따라 다른 값을 가진다.
    sysdate,               -- 함수 > output > 모든 레코드가 동일한 값을 가진다. 너나할거없이 모두 같은 값
...
select name, buseo, rownum from tblInsa where rownum = 3; -- 아무도 안나옴
select name, buseo, rownum from tblInsa where rownum > 5;

-- 1부터 시작해서 나오는 것만 가능하다.
select name, buseo, rownum --2. 1에서 이미 할당된 숫자를 rownum 표현을 통해서 가져온다. (여기서 생성x)
from tblInsa; --1. from절 실행되는 순간 모든 레코드의 rownum이 할당된다.
