...
/*
    ...
    - out 파라미터를 사용하면 안된다. > return 문만 사용
    - in 파라미터는 사용한다.
...
select
    height, weight,
    height + weight,
    fmSum(height, weight) -- 인자 > 반환값 > 반환값 딱 1개 > 값으로 사용
from tblComedian;
...
/*
    ...
    - 특정 테이블 지정 > 지정 테이블 감시(오라클) >
        insert or update or delete > 미리 준비해놓은 프로시저가 자동 호출
...
begin
    ...
    -- 5. 무언가를 학ㅆ다. > 퇴사 직원의 프로젝트를 다른 직원에게 위임
    update tblProject set staff_seq = 4 where staff_seq = :old.seq; -- 삭제되는 직원의 seq값을 받아서 update 실행
...
create or replace procedure procBuseo(
    pbuseo in varchar2,
    pcursor out sys_refcursor -- 커서를 반환값으로 사용할 때 쓰는 자료형 sys_refcursor
)
is
