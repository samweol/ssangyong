...
/*
    ...
    - 릴레이션 = 객체 = 레코드 = 엔티티
        ...
        ERD > Entity, Attribute, Realtion 등 표현하는 방법 > 그림 그리는 방법
    ...
    - 특정 테이블에 데이터를 삽입할 때 원하는 데이터까지 같이 넣어야하는 상황
    
    2. 갱신 이상, Update Abnormaly
    - 동일한 데이터가 2개이상의 테이블에 동시 저장 > 그 중 1개는 수정했지만 다른 1개는 수정하지 못했을 때 상황
    > 두 데이터 간의 이상 오류 발생
    > 즉, 2개의 데이터 중 어떤 데이터가 올바른 데이터인지 구분 불가능
    
    3. 삭제 이상, Delete Abnormaly
    - 특정 테이블의 데이터를 삭제할 때 원하지 않는 데이터까지 같이 지워지는 상황
    
    함수 종속, Functional Dependency
    - 하나의 테이블내 컬럼끼리의 관계 표현
    - 정규화는 '부분 함수 종속'이나 '이행 함수 종속'을 모두 업서애고, 모든 컬럼 관계를 '완전 함수 종속'으로 만드는 작업이다.
    ...
    - 주로 복합키를 가지는 테이블에서 발견되며, 일부 컬럼이 복합키 모두에게 종속이 아니라, 복합키 일부에 종속되는 현상이 발견 
    > 현상 제거
