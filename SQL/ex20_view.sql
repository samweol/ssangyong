...
/*
    ...
    View, 뷰
    - 데이터베이스 객체 중 하나(CREATE, ALTER, DROP > 테이블, 시퀀스,  제약사항)
...
select * from vwSeoul; -- 20명 -- 동일. 이름이 있는 뷰 > 뷰
select * from (select * from tblInsa where city = '서울') -- 동일. 이름이 없는 뷰 > 인라인 뷰
...
-- 뷰 사용
-- 1. SELECT > 실행O > 뷰는 일긱 전용으로 사용한다 (******) == 읽기 전용 테이블
...
insert into 대여체크 values ('홍길동', '영구와 땡칠이', sysdate, '미완료'); -- 오류발생