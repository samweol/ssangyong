...
create table tblComputer(
    ...
    qty number not null, 
    pseq number null references tblComputer(seq) --부모부폼(FK)
);
...
select
    ...
    level
from tblComputer
    --start with seq = 1 -- 결과셋의 루트 레코드를 지정
    --start with seq = (select seq from tblComputer where name = '모니터')
    start with pseq is null -- 컴퓨터
        connect by prior seq = pseq; -- 레코드와 부모 레코드를 연결하는 조건(셀프조인)
