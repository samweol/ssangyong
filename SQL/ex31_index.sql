...
-- 인덱스 없이 검사
select distinct name from tblIndex where name = '홍길동'; --경과 시간: 00:00:00.643

-- 인덱스 생성
create index idxName on tblIndex(name);

-- 인덱스 검색
select distinct name from tblIndex where name = '홍길동'; --경과 시간: 00:00:00.050
...
-- 색인의 값이 중복이 불가능하다.
-- PK, UNIQUE
create unique index idxName on tblIndex(name); -- 동명이인이 한명도 없을 때 
...
select * from tblIndex where buseo = '기획부'; -- 인덱스 효과 o
select * from tblIndex where buseo = '기획부' and name = '홍길동'; -- 인덱스 효과x

-- 복합(결합) 인덱스
-- 컬럼 N개를 대상
create index idxBuseoName on tblIndex(buseo, name);

select * from tblIndex where buseo = '기획부' and name = '홍길동'; -- 인덱스 효과o
-- 데이터 버전에 따라 컬럼 순서가 바뀌면 인덱스 효과가 없는 경우도 있음

-- 함수 기반 인덱스
create index idxSsn on tblIndex(ssn);
create index idxSsn on tblIndex(substr(ssn, 8, 1));

select count(*) from tblIndex where substr(sss, 8, 1) = '1'; --5초
