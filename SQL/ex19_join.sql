...
-- 구매 이력과 상관 없이 > 고객 정보와 함께 구매이력을 가져오시오.
select
    *
from tblCustomer c 
...
-- 대여기록과 무관 > 모든 회원의 이름과 대여 횟수를 가져오시오.
select
    m.name as "고객명",
    count(r.member) as "대여횟수"
from tblMember m
    left outer join tblRent r
        on m.seq = r.member
            group by m.name
                order by count(r.member) desc;
...
insert into tblSelf values (5, '최사원', '개발부', 4);
...
select
    ...
    b.department as "부서",
    a.name as "상사명"
from tblSelf a -- 부모 > 상사 테이블
    inner join tblSelf b -- 자식 > 직원 테이블
        on a.seq = b.super;
...
select
    ...
    a.name as "상사명"
from tblSelf a -- 부모 > 상사 테이블
    right outer join tblSelf b -- 자식 > 직원 테이블
        on a.seq = b.super;
...
select * from employees; -- 매니저 아이디가 employee_id를 참조한다.
...
select
    *
from tblCustomer c
    right outer join tblSales s -- 자식 가리킨 조인 == 내부조인
        on c.seq = s.cseq;
