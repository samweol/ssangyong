...
//		m14();
		...
		// String
		// 1. length() > 문자열 길이
		// 2. charAt(index) > 문자 추출(반환값 char)
		// 3. indexOf(search, index), lastIndexOf(search, index) > 위치 검색
		// 4. trim() > 공백 제거
		// 5. toUppderCase(), toLowerCase() > 대소문자 변환
		// 6. startWith(word), endWith(word) > 패턴 검색
		// 7. substring(begin, end) > 문자열 추출
		// 8. contains(word) > 문자열 검색
		// 9. replace(old, new) > 문자열 치환
		// 10. toCharArray() > 문자열 char 배열로 전환
		// 11. String[] split(String delimiter) > 문자열 분리
	...
	private static void m12() {
		...
		System.out.println(txt.substring(5)); // 자료형 > String > "바사아자차카타파하"
		System.out.println();
		...
		// 몇년생?
		System.out.println(jumin.charAt(0) + jumin.charAt(1)); //110 (char + char = int)
...
//		String filename = path.substring(index+1, path.length()); // 시작 위치~끝 위치
		...
		//"Hel.lo.java" 가 가능하기때문에 뒤에서부터 찾아야 확장자가 나옴.
	...
	private static void m10() {
		...
		System.out.println(name.startsWith("홍")); //true > 가독성이 제일 높음
		System.out.println(name.charAt(0) == '홍'); //true
		System.out.println(name.indexOf('홍') == 0); //true
		System.out.println();
		
		System.out.println(txt.endsWith("프로그램밍")); //true
...
//		img = img.toLowerCase(); > 추천하지 않음 > 검색을 위해서 한 행동 > 아래 조건에 사용되는 목적 이외에 영구적으로 변환
		
		// 해당 파일이 이미지 파일입니까? > .jpg, .jpeg, .jif, .png
		if(img.toLowerCase().endsWith(".jpg") 
				|| img.toLowerCase().endsWith(".jpeg") 
				|| img.toLowerCase().endsWith(".jif") 
	...
	private static void m8() {
		...
		if(content.indexOf(word) > -1) {
			// 금지어를 발견 -> 금지어의 위치가 중요한 것이 아님
			System.out.println("금지어 발견");
		} else {
			System.out.println("글쓰기 진행...");
		}
		
		String[] words = {"바보", "멍청이", "메롱"}; //금지어 리스트
		
		for(int i=0; i<words.length; i++) {
			
			if(content.indexOf(words[i]) > -1) {
				System.out.println("금지어 발견");
				break; //금지어를 발견한 순간 앞으로 검사할 필요가 없으므로 break문 작성
			}
		}
	...
	private static void m7() {
		...
		// - 처음 만난 검색의 위치를 반환한 뒤 종료(뒤에서 반복되는 검색어는 검색 안함)
		// - int indexOf(char c)
...
//		index = txt.indexOf("홍길동", index + 3);
		index = txt.indexOf("홍길동", "홍길동".length()); // 두번째 홍길동
		System.out.println(index);
		
		index = txt.indexOf("홍길동", "홍길동".length()); // 세번째 홍길동
		System.out.println(index);
	}
	...
	private static void m6() {
		...
		// 문자열 공백 제거
		// - String trim()
		// - 문자열에 존재하는 공백(Whitespace > 스페이스, 탭, 개행(엔터))을 제거하는 메소드
	...
	public static boolean isValid2(String name) {
...
//			System.out.println(c);
			...
			// 문자 코드 값 비교 - > 질문의 의도는 잘못된 걸 찾는게 더 좋다.
			if(c <'a' || c>'z') { // 영어 소문가자 아니냐 ?
				System.out.println("잘못된 문자가 발견되었습니다.");
				break; //잘못된 글자가 하나라도 발생하면 for문을 돌 필요가 없음(id로서 가치가 없음)
			}
	...
	private static void m1() throws Exception {
		...
		if(statement.length()>=2 && statement.length()<=5) {
			...
			System.out.println("이름은 2자 ~ 5자 이내로 입력 바랍니다.");
		}
