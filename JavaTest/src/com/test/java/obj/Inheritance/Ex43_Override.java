	...
	public static void main(String[] args) {
		...
		/*
			...
			2. 상속 관계에 있는 클래스의 객체들은 대부분 공통된 사용법(메소드)을 가지고 있다고 생각들을 한다.
				- 스스로 다른 형식의 메소드를 생성 > 규칙 파괴
			3. 겉으로 보기엔 부모의 메소드와 동일한데 내용물 바뀐 메소드를 만들어내자!
			
			면접
			- 메소드 오러로딩 vs 메소드 오버라이딩?
			
		*/
		
		// ** 상속 관계에 있는 클래스 > 같은 목적을 가지는 메소드 구현 >> *** 반드시 메소드 시그니처를 동일하게 만드는게 좋다.
		...
		// > 결론 > 무조건 자신의 hello()가 호출된다.(선택 불가능)
		// > 메소드의 알맹이가 수정된 것 처럼 보인다.
		// > 메소드 재정의 > 메소드 오버라이드
		// > 시대에 맞게 내용을 업그레이드 + 사용법 유지
		c1.hello(); // 외부에서 보면 행동이 바뀐 것 처럼 보인다.
	}
}
