...
//		m6();
	...
	private static void m6() {
		...
		// 변수값의 초기값 > 실패하거나 아무일도 안일어났을 때 가져야될 값으로 초기화한다. 피해가 가장 적을 것 같다고 생각하는 값
		...
		System.out.println(contatins(member, search));
	}
	
	public static boolean contatins(String[] member, String search) {
...
//		Arrays.sort(lotto); // 정렬
		...
		System.out.println(end - begin); // 코드가 실행되는 시간 측정
	}
	...
	private static void m1() {
		...
		// 2차원 배열
		// = new int[2][3];
		System.out.println(nums2.length); //2 > 행의 길이를 반환
		System.out.println(nums2[0].length); //3 > 열의 길이를 반환
