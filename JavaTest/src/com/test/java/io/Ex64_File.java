...
//		m3();
	...
	private static void save(ArrayList<User> list) {
		...
		try {
			...
			System.out.print("Ex64_File.save");
			e.printStackTrace();
		}
	...
	private static void load(ArrayList<User> list) {
		...
		try {
			...
			System.out.print("Ex64_File.load");
			e.printStackTrace();
		}
	...
	private static void m3() {
		...
		try {
			...
			//22살 이상 목록 다시 출력
//			System.out.println(reader.readLine()); //null > 한번 끝까지 읽은 데이터는 다시 읽을 수 없다.
	...
	private static void m1() {
		...
		try {
			...
			/*
				...
				C팀원
				- C:\Users\in\Downloads\data 넣어서 관리
			
			*/
		
			File file = new File("data\\data.txt"); //현재 폴더 > 상대 경로 작성
			
//			System.out.println(file.getAbsolutePath());	//C:\class\java\JavaTest\. > 절대 경로
