...
//		m16();
	...
	private static void m16() {
		...
		// HashSet으로 루프 돌리는 방법으로 (Iterator 사용)
		...
		//HashMap을 돌린 게 아니라 HashMap의 key와 value를 나눠서 각각의 set으로 받은 후 set을 iterator을 활용하여 돌려버린 것
	...
	private static void m15() {
		...
		//파일 11,676, 폴더 2,415 크기 770MB (807,952,715 바이트)
		...
		if(dir.exists()) {
			...
			System.out.printf("총 파일 개수 : %d개, 총 폴더 개수 : %d개\n", count, sum);
	...
	private static void m14() {
		...
		if(dir.exists()) {
			...
			for(File subdir : list) {
				...
				if(subdir.isDirectory()) { //폴더냐고 물어봐야함!
					//자식 폴더의 내용물
	...
	private static void m13() {
		...
		if(dir.exists()) {
			...
			//자식 파일명 + 자식 폴더명
//			String[] list = dir.list(); //이름만 받아옴(파일인지 폴더인지 구분이 안됨)
//			
//			for(String f : list) {
//				
...
//				System.out.println(file.isFile()); //파일true, 폴더false
//				System.out.println();
...
//		System.out.println(c.getActualMaximum(Calendar.DAY_OF_YEAR)); //365 > 총 몇일인지
		
		int date = c.getActualMaximum(Calendar.DAY_OF_YEAR);
...
//		}
		...
		if(!dir.exists()) {
			boolean result = dir.mkdirs(); //mkdir과 mkdirs의 차이점 비교
			System.out.println(result);			
		} else {
			System.out.println("이미 같은 이름의 폴더가 존재함.");
		}
	...
	private static void m5() {
		...
		if(file.exists()) {
			...
			boolean result = file.renameTo(file2); //단순 파일이름 변경이 아닌 전체 경로를 바꾸는 메소드
			System.out.println(result);
		}
	}
	...
	private static void m3() {
		...
		if(!file.exists()) {
			...
			try {
				System.out.println(file.createNewFile()); //true > 새로운 파일 만드는 메소드
	...
	private static void m1() {
		...
		//System.out.println(file.exists()); //true > 실제 경로의 파일이 존재하는지를 검사해주는 메소드
		...
		if(file.exists()) {
			...
			System.out.println(file.lastModified());		//1649031844960 > 수정한 날짜 > tick 값
