...
public class Ex74_Lambda {
	...
	public static void main(String[] args) {
		...
		/*
			...
			함수형 인터페읏, Functional Interface
			- 람다식을 사용하기 위한 인터페이스 제공
			- 다른 용도로 사용 안함
...
//		aaa(hong, c1, c2);
		...
		//c1 + c2 + c4
		Consumer<User> c5 = c3.andThen(c4);
		c5.accept(hong);
		System.out.println();
		System.out.println();
		...
		Function<Integer, Integer> f9 = f7.compose(f6); //f7 + f6
	...
	public static void aaa(User hong, Consumer<User> c1, Consumer<User> c2) {
		c1.accept(hong);
		c2.accept(hong);
	}
...
//함수형 인터페이스(목적 : 람다식을 저장하려고 만든 인터페애스)
//> 함수형 인터페이스는 반드시 추상 메소드를 딱 1개만 가질 수 있다. > 이 1개의 추상 메소드가 곧 람다식(익명 메소드)이 되기때문에 ..
