...
public class reservationDAO {
   ...
   public reservationDTO get(String seq) {
	...
	try {
		...
		String sql = "select r.hpseq seq, a.name name, v.visit visit, r.resdate resdate, p.purpose purpose, u.name username, u.tel tel, r.uniqueness uniqueness from tblanimal a inner join tbluserani e on a.aseq = e.aseq inner join tbluser u on u.useq = e.useq inner join tblreshos r on e.uaseq = r.uaseq inner join tblvisit v on v.visitseq = r.visitseq inner join tblpurpose p on p.pseq = r.pseq where r.hpseq = ?";
