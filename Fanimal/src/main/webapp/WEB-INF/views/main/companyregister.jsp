...
<style>
	#homelogo {
	        margin: 0 auto;
	        text-align: center;
	        background-image: url('/fanimal/asset/images/companylogo.png');
...
<body>
	<main>
		<div class="content">
        	...
        	<div style="display: flex; justify-content: center; margin-left: 50px;">
	        	<form action="/fanimal/main/companyregister.do" method="post" id="formData" style="display: flex; flex-direction: column;">
		        	...
		        	<div style="display:flex; flex-direction: column; padding-bottom: 10px;">
			        	<div style="display: flex;">
			        		<div class="cal">사업자번호</div>
			        		<input type="text" id="business" name="business" style="margin-right: 5px;" required>
			        		<input type="button" value="검사" id="business-check">
			        	</div>	
		        		<div style="display:flex; justify-content: center;" id="business-check-result" class="error"></div>
		        	</div>	
		        	<div class="col">
		        	...
		        	<div class="col">
		        		<div class="cal">이메일</div>
		        		<input type="text" name="email1" style="width: 100px;" required> @ 
		        		<input type="text" name="email2" style="width: 100px;" required>
		        	</div>
		        	<div>
	...
	<script>
	    ...
	    $('#business-check').click(function() {
	    	$.ajax({
	    		url: '/fanimal/main/businesscheck.do',
	    		type: 'POST',
	    		...
	    		success: function(result) {
	    			if(result.result == "1") {
	    				//비즈니스 번호가 존재하는 경우
	    				$('#business-check-result').text('중복된 사업자번호가 존재합니다.');
	    				$('#business-check-result').css('color', 'red');
	    				isValidId = false;
	    			} else {
	    				//비즈니스 번호가 존재하지 않아 사용가능한 경우
	    				if(!/^[0-9]{13}$/.test($('#business').val())) {
			            	$('#business-check-result').css('color', 'red');
			            	$('#business-check-result').text('사업자번호는 13자리 입니다.');
			            	isValidId = false;
			            } else {
		    				$('#business-check-result').text('확인되었습니다.');
		    				$('#business-check-result').css('color', 'green');
		    				isValidBusiness = true;	
			            }
	    			}
	    			
	    			if (isValidId && isValidBusiness) {
