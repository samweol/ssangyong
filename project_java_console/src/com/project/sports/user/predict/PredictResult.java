...
public class PredictResult {
	...
	public static void predictResult() {
		...
		for(Predict p : Data.predictList) {
			...
			//아이디 dlhl9ry
			if(UserLogin.userData.getId().equals(p.getId())) { //아이디가 일치하는 경우를 찾아와야한다.
				predictTeam = p.getTeamName(); //승리예측한 팀
				predictResult = p.getResult(); //예측 결과(1/2/3)
				int gameSeq = p.getScheduleSeq(); //경기고유번호
						
				//아이디가 일치한 상태 + schedule에서 일치한 경기 가져오기
				for(Schedule s : Data.scheduleList) {
					
					if(s.getSeq() == gameSeq) { //승부예측한 경기의 팀vs팀에서 이름을 얻어오기 위해서
						teamName1 = s.getTeam1();
						teamName2 = s.getTeam2();
						date = String.format("%tF",s.getDate());
						
						if(monday.compareTo(s.getDate())<0 && c.compareTo(s.getDate())>0) {
				...
				if(date.equals(yesterday)) {
					...
					String line = String.format("%3d\t%8svs%8s\t%8s\t%3s\t%3dp"
							, seq
							, teamName1
							, teamName2
							, predictTeam
					...
					inputPointList(point); //리스트에 포인트 추가하기(하나씩 해줘야함 - 위치가여기가 맞음?)
					seq++;
	...
	public static void printPoint(int pointSum, int weekPoint, int monthPoint) {
		...
		System.out.println("계속하시려면 (Enter)을 누르세요.");
		
		//seq●id●point●"경기 예측 보상" - 고정
		
	}
	
	public static void inputPointList(int point) {
		//얻은 포인트 -> 포인트 내역 리스트에 넣기
		int last = Data.pointList.get(Data.pointList.size()-1).getSeq();
		Point inputPoint = new Point(last+1, UserLogin.userData.getId(), point, "경기 예측 보상");
